    ...
    $('#Tableservice_type').DataTable({
        ...
        buttons: [
            ...
            },
            {
                ...
                attr: {
                    ...
                    'data-bs-target': '#offcanvasAddservice_type',
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, full, meta) {
                    if (data === 'it') {
                        return '<span class="badge bg-label-primary">IT</span>';
                    } else {
                        return '<span class="badge bg-label-success">Design</span>';
                    }
                }
            },
            {
                ...
                render: function (data, type, full, meta) {
                  ...
                  let canEdit         = userPermissions.includes("edit_services");
                  let canDelete       = userPermissions.includes("delete_services");
    ...
    window.ViewData = function (id) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    selectedId = id;
                    $("#id").val(response.service_type.id);
                    $("#name").val(response.service_type.name);
                    $("#type").val(response.service_type.type);
                    $('#offcanvasAddservice_typeLabel').text('Edit Service Type');
                    // Ubah tombol submit agar tahu ini update
                    $(".data-submit").text("Update").attr("id", "updateMenu");
                    $("#offcanvasAddservice_type").offcanvas("show");
                }
            },
            error: function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $("#offcanvasAddservice_type").offcanvas("hide");
