...
$(document).ready(function () {
        ...
        columns: [{
            ...
            },
            {
                ...
                render: function (data, type, full, meta) {
                  ...
                  let canEdit         = userPermissions.includes("edit_organisasi");
                  let canDelete       = userPermissions.includes("delete_organisasi");
    ...
    $('#formOrganisasi').on('submit', function(e){
        ...
        $('#formOrganisasi .text-danger.small').text('');
        ...
        if(id){
            url = '/internal/organisasi/update/' + id;
            method = 'POST';
            formData.append('_method', 'PUT');
        } else {
            url = '/internal/organisasi/store';
            method = 'POST';
        }
...
function editOrganisasi(id) {
    ...
    $.ajax({
        url: '/internal/organisasi/edit/' + id,
        type: 'GET',
        success: function(response) {
            if (response.success) {
                let organisasi = response.organisasi;
                $('#id').val(organisasi.id);
                $('#nama').val(organisasi.nama);
                $('#jabatan').val(organisasi.jabatan);

                // Set content to TinyMCE editor
                if (tinymce.get('deskripsi')) {
                    tinymce.get('deskripsi').setContent(organisasi.deskripsi || '');
                } else {
                    $('#deskripsi').val(organisasi.deskripsi);
                }

                $('#lokasi').val(organisasi.lokasi);
                $('#tahun').val(organisasi.tahun);
...
$(document).on('click', '.delete-record', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/internal/organisasi/delete/' + id,
                type: 'DELETE',
                data: {
                    _method: 'DELETE',
